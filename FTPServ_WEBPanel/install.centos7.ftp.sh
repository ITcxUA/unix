...
##=============================================
ASK="Произвести установку FTPD и Простую настройку ftp на CentOS 7"
while true; do
...
tar czvf /etc/vsftpd_"$DATA_TIME".backup.tar.gz /etc/vsftpd
rm -rf /etc/vsftpd/*
...
passwd ftpuser $USERPASS
echo "USER: $UNAME ; PASS: $USERPASS " >> ~/FTPUser_NamePass.txt;
...
## отключить проверку оболочки в настройках pam для /etc/pam.d/vsftpd
#auth required pam_shells.so
#sed -i 's/^auth required .*/#auth required pam_shells.so/g' /etc/pam.d/vsftpd
	cat /etc/pam.d/vsftpd > /etc/pam.d/vsftpd."$DATA_TIME".backup
...
## ==========================================
##### Продолжаем настройку ftp сервера. 
## каталог для персональных настроек пользователей:
mkdir -p /etc/vsftpd/users

## Cоздадим файл с именем пользователя ftpuser и укажем его домашний каталог:
...
# файл пользователями можно выходить за пределы домашнего каталога:
