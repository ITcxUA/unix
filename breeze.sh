...
exist=`whereis $1 | awk {
'print $2'
}` #вариант быстрый, но не всегда эффективный
if [-z $exist]
...
case "$osver1" in
4|5|6)
ip=`ifconfig $iface | grep 'inet addr' | awk {
'print $2'
} | sed s/.*://`
;;
7)
ip=`ifconfig $iface | grep 'inet' | sed q | awk {
'print $2'
}`
;;
*)
...
ipext=`wget --no-check-certificate -qO- https://2ip.ru/index.php | grep "Ваш IP адрес:" | sed s/.*button\"\>// | sed s_"<"_" "_ | awk {
'print $1'
}`
}
...
read int
ip=`ifconfig $int | grep 'inet addr' | awk {
'print $2'
} | sed s/.*://`
#centOS7
if [$osver1 -eq 7]; then ip=`ifconfig $int | grep 'inet' | sed q | awk {
'print $2'
}`; fi
echo "Ваш внешний IP: $ip?"
myread_yn ans
...
totalspeed = $(sysbench cpu --cpu-max-prime = 10000 run --num-threads = $threads | grep "events per second:" | awk {
'print $4'
}) #записали общую скорость
temp = $(echo "$ {
totalspeed/./
}") #убрали точку, т.е. умножили на 100.
if [$ {
temp:0:1
} -eq 0]; then temp = $(echo "$ {
temp:1
}"); fi #проверили нет ли нуля в начале, если есть - убрали
...
if [$ {
#mb
} -eq 1]; then let "mb=$mb*1024/10"; else #переводим десятые доли гигабайт в мегабайты
if [$ {
#mb
} -eq 2]; then let "mb=$mb*1024/100"; else #переводим сотвые долги гигабайт в мегабайты
if [$ {
#mb
} -eq 3]; then let "mb=$mb*1024/1000"; else #переводим тысячные долги гигабайт в мегабайты
mb = 0
fi
fi
fi
...
if [$ {
#iface
} -eq 4]; then #проверяем какой сетевой интерфейс. Если мы его не определили, то вообще не выводим эту строку
...
cpu_clock=`cat /proc/cpuinfo | grep "cpu MHz" | awk {
'print $4'
} | sed q`
...
#Определяем ОС
if ["$(cat /etc/redhat-release | awk {
'print $2'
})" == "release"]
then
osfamily = $(cat /etc/redhat-release | awk {
'print $1'
})
osver2 = $(cat /etc/redhat-release | awk {
'print $3'
})
else
if ["$(cat /etc/redhat-release | awk {
...
case "$ans" in
y|Y)
usedspace1=`df | awk '(NR == 2)' | awk {
'print $3'
}` #запоминаем сколько было занято места до очистки
...
usedspace2=`df | awk '(NR == 2)' | awk {
'print $3'
}` #смотрим сколько занято теперь
